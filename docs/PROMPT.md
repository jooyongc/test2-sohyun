...
7. **참여 기능 (댓글 · 좋아요 · SNS 공유)** —
   - **댓글**: 익명 방문자가 이름+내용으로 작성(공개). 관리자 로그인 시 삭제 가능.
   - **좋아요**: 로그인 없이 브라우저 단위 토글. `localStorage`의 `visitor_id`로 중복 방지,
     좋아요 수 표시.
   - **SNS 공유**: `navigator.share`(모바일 공유 시트, 카카오톡 등) 우선 + X/Facebook/
     링크복사 폴백.

**데이터 모델:**
- `posts`: `id uuid PK`, `title text NOT NULL`, `content text`, `location text`,
  `cover_image_url text`, `images text[]`, `created_at`, `updated_at`(트리거 자동 갱신).
- `comments`: `id uuid PK`, `post_id uuid FK(on delete cascade)`, `author_name text`,
  `body text`, `created_at`.
- `likes`: `id uuid PK`, `post_id uuid FK(on delete cascade)`, `visitor_id text`,
  `created_at`, `unique(post_id, visitor_id)`.

**보안 (RLS):**
- `posts`: 공개 `SELECT`, `INSERT/UPDATE/DELETE`는 `authenticated`만.
- Storage `post-images`: 공개 읽기, 업로드/수정/삭제는 `authenticated`만.
- `comments`: 공개 `SELECT`+`INSERT`(익명), `DELETE`는 `authenticated`만.
- `likes`: 공개 `SELECT`+`INSERT`+`DELETE`(익명 토글).
...
- 상세 페이지: 좋아요 토글(새로고침 유지) · 댓글 작성/표시/관리자 삭제 · SNS 공유 동작.
