...
masonry gallery; the **admin** signs in to publish posts and upload photos.
...
2. **SQL Editor** → paste & run [`supabase/schema.sql`](supabase/schema.sql).
   This creates the `posts` table, RLS policies, and the `post-images` storage bucket.
...
├── hooks/usePosts.js      Data fetching for posts
...
supabase/schema.sql        DB tables + RLS + storage policies
